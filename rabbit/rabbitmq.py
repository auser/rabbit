...
class RabbitMQ(object):
    ...
    def _load_from_environment(self):
        ...
        logger.debug(
            f"  From: {self.from_username}@{self.from_host}:{self.from_port} (SSL: {self.from_ssl})"
        )
        logger.debug(
            f"  To: {self.to_username}@{self.to_host}:{self.to_port} (SSL: {self.to_ssl})"
        )
    ...
    async def download(self, output: Optional[str | Path] = None):
        ...
        async with httpx.AsyncClient() as client:
            ...
            kwargs = {
                "auth": (self.from_username, self.from_password),
                "headers": headers,
            }
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                try:
                    ...
                    logger.debug(
                        f"API request successful, received {len(result) if isinstance(result, list) else 'data'}"
                    )
                    return result
            ...
            logger.error(f"Error making API request to: {url}")
            # raise Exception(f"Error making API request: {e}")
            return []

    async def _collect_server_data(
        self,
        host: str,
        port: int,
        username: str,
        ...
        # Collect all data
        overview = await self._make_api_request(
            "overview", host, port, username, password, ssl
        )
        queues = await self._make_api_request(
            f"queues/{vhost_encoded}", host, port, username, password, ssl
        )
        exchanges = await self._make_api_request(
            f"exchanges/{vhost_encoded}", host, port, username, password, ssl
        )
        consumers = await self._make_api_request(
            f"consumers/{vhost_encoded}", host, port, username, password, ssl
        )
        bindings = await self._make_api_request(
            f"bindings/{vhost_encoded}", host, port, username, password, ssl
        )
        connections = await self._make_api_request(
            "connections", host, port, username, password, ssl
        )
        channels = await self._make_api_request(
            "channels", host, port, username, password, ssl
        )
        nodes = await self._make_api_request(
            "nodes", host, port, username, password, ssl
        )
    ...
    def _prepare_queues_data(self, queues: List[Dict]) -> pd.DataFrame:
        ...
        for queue in queues:
            ...
            queue_info = {
                ...
                "Message Rate/sec": message_stats.get("publish_details", {}).get(
                    "rate", 0
                ),
                "Consumer Utilisation": queue.get("consumer_utilisation", ""),
                "Node": queue.get("node", ""),
                "Arguments": (
                    json.dumps(queue.get("arguments", {}))
                    if queue.get("arguments")
                    else ""
                ),
    ...
    def _prepare_exchanges_data(self, exchanges: List[Dict]) -> pd.DataFrame:
        ...
        for exchange in exchanges:
            ...
            exchange_info = {
                ...
                "Message Rate In/sec": message_stats.get("publish_in_details", {}).get(
                    "rate", 0
                ),
                "Message Rate Out/sec": message_stats.get(
                    "publish_out_details", {}
                ).get("rate", 0),
                "Arguments": (
                    json.dumps(exchange.get("arguments", {}))
                    if exchange.get("arguments")
                    else ""
                ),
    ...
    def _prepare_consumers_data(self, consumers: List[Dict]) -> pd.DataFrame:
        ...
        for consumer in consumers:
            ...
            consumer_info = {
                ...
                "Arguments": (
                    json.dumps(consumer.get("arguments", {}))
                    if consumer.get("arguments")
                    else ""
                ),
            }
    ...
    def _prepare_bindings_data(self, bindings: List[Dict]) -> pd.DataFrame:
        ...
        for binding in bindings:
            binding_info = {
                ...
                "Arguments": (
                    json.dumps(binding.get("arguments", {}))
    ...
    def _prepare_connections_data(self, connections: List[Dict]) -> pd.DataFrame:
        ...
        for conn in connections:
            conn_info = {
                ...
                "Type": conn.get("type", ""),
                "Client Properties": (
                    json.dumps(conn.get("client_properties", {}))
                    if conn.get("client_properties")
                    else ""
                ),
    ...
    async def inspect_server(
        ...
        # Collect data
        data = await self._collect_server_data(
            host, port, username, password, vhost, ssl
        )

        # Test connection
        if not data["server_info"]["overview"]:
            raise Exception(
                "Could not connect to RabbitMQ server. Please check your connection details."
            )
    ...
    async def compare_servers(
        ...
        prod_data = await self._collect_server_data(
            prod_host, prod_port, prod_username, prod_password, "/", ssl
        )

        logger.info("Collecting development data...")
        dev_data = await self._collect_server_data(
            dev_host, dev_port, dev_username, dev_password, "/", ssl
        )
    ...
    async def _export_comparison_to_excel(self, comparison_data: Dict, filename: str):
        ...
        with pd.ExcelWriter(filename, engine="openpyxl") as writer:
            ...
            # Production sheets
            self._prepare_summary_data(prod_data).to_excel(
                writer, sheet_name="Prod Summary", index=False
            )
            self._prepare_queues_data(prod_data["queues"]).to_excel(
                writer, sheet_name="Prod Queues", index=False
            )
            self._prepare_exchanges_data(prod_data["exchanges"]).to_excel(
                writer, sheet_name="Prod Exchanges", index=False
            )
            self._prepare_consumers_data(prod_data["consumers"]).to_excel(
                writer, sheet_name="Prod Consumers", index=False
            )
            self._prepare_bindings_data(prod_data["bindings"]).to_excel(
                writer, sheet_name="Prod Bindings", index=False
            )
            self._prepare_connections_data(prod_data["connections"]).to_excel(
                writer, sheet_name="Prod Connections", index=False
            )

            # Development sheets
            self._prepare_summary_data(dev_data).to_excel(
                writer, sheet_name="Dev Summary", index=False
            )
            self._prepare_queues_data(dev_data["queues"]).to_excel(
                writer, sheet_name="Dev Queues", index=False
            )
            self._prepare_exchanges_data(dev_data["exchanges"]).to_excel(
                writer, sheet_name="Dev Exchanges", index=False
            )
            self._prepare_consumers_data(dev_data["consumers"]).to_excel(
                writer, sheet_name="Dev Consumers", index=False
            )
            self._prepare_bindings_data(dev_data["bindings"]).to_excel(
                writer, sheet_name="Dev Bindings", index=False
            )
            self._prepare_connections_data(dev_data["connections"]).to_excel(
                writer, sheet_name="Dev Connections", index=False
            )
    ...
    async def _export_comparison_to_csv(self, comparison_data: Dict, prefix: str):
        ...
        prod_datasets = {
            ...
            "prod_connections": self._prepare_connections_data(
                prod_data["connections"]
            ),
        }
    ...
    def _prepare_comparison_summary(self, comparison_data: Dict) -> pd.DataFrame:
        ...
        prod_exchanges = {
            e["name"]
            for e in prod_data["exchanges"]
            if not e["name"].startswith("amq.")
        }
        dev_exchanges = {
            e["name"] for e in dev_data["exchanges"] if not e["name"].startswith("amq.")
        }

        # Get consumers by queue
        prod_consumer_queues = {
            c.get("queue", {}).get("name") for c in prod_data["consumers"]
        }
        dev_consumer_queues = {
            c.get("queue", {}).get("name") for c in dev_data["consumers"]
        }
        ...
        summary_data = [
            ...
            [
                "Generated At",
                comparison_data["comparison_info"]["generated_at"],
                "",
                "",
            ],
            [
                "Production Host",
                comparison_data["comparison_info"]["production"]["host"],
                "",
                "",
            ],
            [
                "Development Host",
                comparison_data["comparison_info"]["development"]["host"],
                "",
                "",
            ],
            ["", "", "", ""],  # Empty row
            [
                ...
                len(prod_queues),
                len(dev_queues),
                len(prod_queues) - len(dev_queues),
            ],
            [
                "Total Exchanges",
                len(prod_exchanges),
                len(dev_exchanges),
                len(prod_exchanges) - len(dev_exchanges),
            ],
            [
                "Total Consumers",
                len(prod_data["consumers"]),
                len(dev_data["consumers"]),
                len(prod_data["consumers"]) - len(dev_data["consumers"]),
            ],
            [
                "Total Connections",
                len(prod_data["connections"]),
                len(dev_data["connections"]),
                len(prod_data["connections"]) - len(dev_data["connections"]),
            ],
            ["", "", "", ""],  # Empty row
            ...
            [
                "Consumer Queue Diff",
                len(prod_consumer_queues - dev_consumer_queues),
                "",
                "",
            ],
        ]
        ...
        if consumer_diff:
            summary_data.append(["", "", "", ""])
            summary_data.append(
                ["Queues with Consumers Only in Production:", "", "", ""]
            )
            for queue in sorted(consumer_diff):
                summary_data.append([f"  - {queue}", "", "", ""])

        return pd.DataFrame(
            summary_data, columns=["Metric", "Production", "Development", "Difference"]
        )
    ...
    def _print_comparison_summary(self, comparison_data: Dict):
        ...
        prod_consumer_queues = {
            c.get("queue", {}).get("name")
            for c in prod_data["consumers"]
            if c.get("queue", {}).get("name")
        }
        dev_consumer_queues = {
            c.get("queue", {}).get("name")
            for c in dev_data["consumers"]
            if c.get("queue", {}).get("name")
        }
        ...
        print("=" * 80)
        print(
            f"Production:  {comparison_data['comparison_info']['production']['host']}"
        )
        print(
            f"Development: {comparison_data['comparison_info']['development']['host']}"
        )
